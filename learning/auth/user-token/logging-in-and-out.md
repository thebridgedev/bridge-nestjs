...
That reframes the interesting question for a backend package: not "how does a user log in," but "what makes a request trustworthy," on every single call. That's `BridgeAuthGuard` and `JwksService`.
...
3. **The JWKS keyset itself is cached, not fetched per request** — one hour TTL, refreshed lazily the first time it's needed after expiry. User-JWT verification and API-token verification use **separate JWKS clients with independent caches**, so rotating one doesn't force-refetch the other.
4. **No revocation list is consulted.** The guard's own JWKS verification only checks the four things above — it does not call back to `bridge-api` per request to ask "has this specific token been revoked." A token remains acceptable to the guard for as long as it's unexpired and correctly signed, full stop.
...
| `TOKEN_INVALID` | `invalid_token` | Malformed token, or (API-token path) wrong `type` claim |

...
- Neither is push-live the way a frontend reactive store is. If your app needs to react the instant something changes server-side, that reaction has to live on the frontend (or via Bridge webhooks; see [Multi-tenancy](/auth/multi-tenancy/)), not inside this guard.

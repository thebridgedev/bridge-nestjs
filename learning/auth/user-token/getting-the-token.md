...
## Tenant-scoped data beyond the JWT: `BridgeService`

`@CurrentTenant()` only gives you what's baked into the JWT (id, name, locale, logo, onboarded). For subscription plan, entitlements, or branding, use the injectable `BridgeService`, which fetches (and short-TTL-caches) a fuller snapshot for the token's tenant:

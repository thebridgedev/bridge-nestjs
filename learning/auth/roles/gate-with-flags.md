...
`BridgeFlagGuard` doesn't verify identity itself — pair it with `BridgeAuthGuard` (or your own auth) so `req.bridgeUser`/`req.bridgeApiToken` is populated first, same requirement as the attribute provider above.
